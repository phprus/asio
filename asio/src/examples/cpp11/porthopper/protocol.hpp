...
public:
  ...
  {
    std::istringstream is(std::string(data_ + encoded_port_size, encoded_port_size));
    unsigned short port = 0;
    is >> std::setw(encoded_port_size) >> std::hex >> port;
    return port;
  }
...
private:
      ...
      unsigned short new_port_number)
  {
    ...
    auto str = os.str();
    std::copy_n(str.begin(), std::min<std::size_t>(str.size(), control_request_size), data_);
  }
...
public:
  ...
  frame(unsigned long frame_number, const std::string& payload_data)
  {
    ...
    auto str = os.str();
    std::copy_n(str.begin(), std::min<std::size_t>(str.size(), frame_size), data_);
  }
