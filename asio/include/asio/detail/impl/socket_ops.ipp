...
signed_size_type recvfrom(socket_type s, buf* bufs, size_t count,
    int flags, void* addr, std::size_t* addrlen, std::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
    ...
    unsigned long scope_id, std::error_code& ec)
{
  ...
  if (result != 0 && af == ASIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
    ...
    {
      // "%lu"
      auto ret = std::to_chars(if_name + 1, std::end(if_name) - 1, scope_id);
      *ret.ptr = '\0';
    }
    strcat(dest, if_name);
  }
    ...
    std::error_code& ec)
{
  ...
  if (serv && servlen > 0)
  {
    ...
    {
      // "%u"
      auto ret = std::to_chars(serv, serv + servlen - 1, ntohs(port));
      if (ret.ec != std::errc())
      {
...
std::error_code getnameinfo(const void* addr,
...
std::error_code sync_getnameinfo(const void* addr,
