...
public:
  // Default constructor.
  ASIO_DECL endpoint() noexcept(true);
  ...
  // Copy constructor.
  endpoint(const endpoint& other) noexcept(true)
    : data_(other.data_)
  {
  }

  // Assign from another endpoint.
  endpoint& operator=(const endpoint& other) noexcept(true)
  {
    data_ = other.data_;
    return *this;
  }
  ...
  asio::detail::socket_addr_type* data() noexcept(true)
  {
    return &data_.base;
  }

  // Get the underlying endpoint in the native type.
  const asio::detail::socket_addr_type* data() const noexcept(true)
  {
    return &data_.base;
  }
  ...
  ASIO_DECL unsigned short port() const noexcept(true);

  // Set the port associated with the endpoint.
  ASIO_DECL void port(unsigned short port_num) noexcept(true);

  // Get the IP address associated with the endpoint.
  ASIO_DECL asio::ip::address address() const noexcept(true);

  // Set the IP address associated with the endpoint.
  ASIO_DECL void address(
      const asio::ip::address& addr) noexcept(true);

  // Compare two endpoints for equality.
  ASIO_DECL friend bool operator==(const endpoint& e1,
      const endpoint& e2) noexcept(true);
